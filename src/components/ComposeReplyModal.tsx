...
interface ComposeReplyModalProps {
  tweetReplyTo: Tweet
  isOpen: boolean
  setIsOpen: React.Dispatch<React.SetStateAction<boolean>>
}

function ComposeReplyModal({
  tweetReplyTo,
  isOpen,
  setIsOpen
}: ComposeReplyModalProps) {
  ...
  const date = getRelativeDate(tweetReplyTo.createdAt)
  ...
  const submitHandle = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (errors) {
      ...
      setErrors([])
      setIsOpen(false)
      navigate(`/tweet/${data.ReplyTweet.id}`)
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className='fixed inset-0 top-0 p-4'>
        <Dialog.Panel className='mx-auto max-w-lg w-full rounded-xl bg-black p-4'>
          ...
          <div
            ref={tweetReplyToRef}
            className='flex items-start gap-2 w-full relative min-h-[80px]'
          >
            <div
              ref={barTweetReplyToRef}
              className='w-[3px] bg-zinc-600 rounded-full absolute left-[22px] top-[52px]'
            />

            <Link to={`/profile/${tweetReplyTo.user.username}`}>
              <img
            ...
            <div className='flex-1'>
              <div className='flex items-center gap-2 text-base text-white'>
                <span className='font-bold hover:underline'>
                  {tweetReplyTo.user.name}
                </span>

                <span className=' text-zinc-500'>
                  @{tweetReplyTo.user.username} · {date}
                </span>
              </div>

              <p className='text-base break-all'>{tweetReplyTo.text}</p>
            </div>
          </div>
